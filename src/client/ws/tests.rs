...
use crate::common::{JsonValue, Params, Error, ErrorCode};
use crate::client::{WsClient, WsRequestError};

use jsonrpsee_test_utils::helpers::*;
use jsonrpsee_test_utils::types::{Id, WebSocketTestServer};

fn assert_error_response(response: Result<JsonValue, WsRequestError>, code: ErrorCode, message: String) {
    match response {
...
async fn method_call_works() {
    let server = WebSocketTestServer::with_hardcoded_response("127.0.0.1:0".parse().unwrap(), ok_response("hello".into(), Id::Num(0_u64))).await;
    let uri = to_ws_uri_string(server.local_addr());
    let client = WsClient::new(&uri).await.unwrap();
    let response: Result<JsonValue, WsRequestError> = client.request("say_hello", Params::None).await;
...
async fn method_not_found_works() {
    let server = WebSocketTestServer::with_hardcoded_response("127.0.0.1:0".parse().unwrap(), method_not_found(Id::Num(0_u64))).await;
    let uri = to_ws_uri_string(server.local_addr());
    let client = WsClient::new(&uri).await.unwrap();
    let response: Result<JsonValue, WsRequestError> = client.request("say_hello", Params::None).await;
...
async fn parse_error_works() {
    let server = WebSocketTestServer::with_hardcoded_response("127.0.0.1:0".parse().unwrap(), parse_error(Id::Num(0_u64))).await;
    let uri = to_ws_uri_string(server.local_addr());
    let client = WsClient::new(&uri).await.unwrap();
    let response: Result<JsonValue, WsRequestError> = client.request("say_hello", Params::None).await;
...
async fn invalid_request_works() {
    let server = WebSocketTestServer::with_hardcoded_response("127.0.0.1:0".parse().unwrap(), invalid_request(Id::Num(0_u64))).await;
    let uri = to_ws_uri_string(server.local_addr());
    let client = WsClient::new(&uri).await.unwrap();
    let response: Result<JsonValue, WsRequestError> = client.request("say_hello", Params::None).await;
...
async fn invalid_params_works() {
    let server = WebSocketTestServer::with_hardcoded_response("127.0.0.1:0".parse().unwrap(), invalid_params(Id::Num(0_u64))).await;
    let uri = to_ws_uri_string(server.local_addr());
    let client = WsClient::new(&uri).await.unwrap();
    let response: Result<JsonValue, WsRequestError> = client.request("say_hello", Params::None).await;
...
async fn internal_error_works() {
    let server = WebSocketTestServer::with_hardcoded_response("127.0.0.1:0".parse().unwrap(), internal_error(Id::Num(0_u64))).await;
    let uri = to_ws_uri_string(server.local_addr());
    let client = WsClient::new(&uri).await.unwrap();
    let response: Result<JsonValue, WsRequestError> = client.request("say_hello", Params::None).await;
