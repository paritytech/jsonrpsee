			...
			b.to_async(rt).iter(|| async {
				...
				for _ in 0..(size / 3) {
					for method in request.methods() {
						futs.push(client.request::<String, ArrayParams>(method, rpc_params![]));
					}
				}
...
fn sub_round_trip(rt: &TokioRuntime, crit: &mut Criterion, client: Arc<impl SubscriptionClientT>, name: &str) {
	...
	group.bench_function("subscribe", |b| {
		b.to_async(rt).iter_with_large_drop(|| async {
			black_box(
				client
					.subscribe::<String, ArrayParams>(SUB_METHOD_NAME, rpc_params![], UNSUB_METHOD_NAME)
					.await
					.unwrap(),
			);
		})
				...
				|clients| async {
					let tasks = clients.into_iter().map(|client| {
						rt.spawn(async move {
							...
							for _ in 0..10 {
								futs.push(client.request::<String, ArrayParams>(fast_call, rpc_params![]));
							}
